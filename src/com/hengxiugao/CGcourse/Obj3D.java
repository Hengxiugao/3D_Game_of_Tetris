...
{  private float rho, d, theta=0.30F, phi=1.3F, rhoMin, rhoMax,
   ...
   public void ChangeWorldCoor()
   {
	   Vector neww = new Vector();
	   neww.ensureCapacity(w.size());
	   Point3D a[] = new Point3D[9];
	   a[1] = new Point3D(5,10,2);
	   a[2] = new Point3D(6,10,2);
	   a[3] = new Point3D(6,11,2);
	   a[4] = new Point3D(5,11,2);
	   a[5] = new Point3D(5,10,3);
	   a[6] = new Point3D(6,10,3);
	   a[7] = new Point3D(6,11,3);
	   a[8] = new Point3D(5,11,3);
	   ...
	   for(int i=0;i<w.size();i++)
	   {
		   ...
		   if (P == null)
	         ...
	         {  
	        	 System.out.println("i="+i+"World coor: X="+P.x+", Y="+P.y+", Z="+P.z);
	        	 if(i>=neww.size())
	        		 neww.setSize(i+1);
	        	 neww.setElementAt(a[i], i);
	         }
	   }
	   w = neww;
   }
