...
class ObjCube extends Obj3D
{  public float rho, d, theta=0.30F, phi=1.3F, rhoMin, rhoMax,
   ...
   public void ChangeWorldCoor(int xi,int yi,int zi)
   {
	   w.set(1,new Point3D( 2+xi, 0+yi, 0+zi));
	   w.set(2,new Point3D( 2+xi, 2+yi, 0+zi));
	   w.set(3,new Point3D( 0+xi, 2+yi, 0+zi));
	   w.set(4,new Point3D( 0+xi, 0+yi, 0+zi));
	   w.set(5,new Point3D( 2+xi, 0+yi, 2+zi));
	   w.set(6,new Point3D( 2+xi, 2+yi, 2+zi));
	   w.set(7,new Point3D( 0+xi, 2+yi, 2+zi));
	   w.set(8,new Point3D( 0+xi, 0+yi, 2+zi));
	   
   }
   public void printCoor()
   {
	   System.out.println("Print Coordinate:");
	   for(int i=0;i<w.size();i++)
	   {
		   ...
		   if (P == null)
	         ...
	         {  
	        	 System.out.println("World coor: X="+P.x+", Y="+P.y+", Z="+P.z);
	         }
	   }
	   ...
	   for(int i=0;i<e.length;i++)
	   {
		   ...
		   if (P == null)
	         ...
	         {  
	        	 System.out.println("Eye coor: X="+P.x+", Y="+P.y+", Z="+P.z);
	         }
	   }
   }
   ...
   {
	   w.setSize(9);
	   w.setElementAt(new Point3D( 2, 0, 2), 0);
	   
	   int xi=18,yi=9,zi=0;
	   w.setElementAt(new Point3D( 2+xi, 0+yi, 0+zi), 1);
	   w.setElementAt(new Point3D( 2+xi, 2+yi, 0+zi), 2);
	   w.setElementAt(new Point3D( 0+xi, 2+yi, 0+zi), 3);
	   w.setElementAt(new Point3D( 0+xi, 0+yi, 0+zi), 4);
	   
	   w.setElementAt(new Point3D( 2+xi, 0+yi, 2+zi), 5);
	   w.setElementAt(new Point3D( 2+xi, 2+yi, 2+zi), 6);
	   w.setElementAt(new Point3D( 0+xi, 2+yi, 2+zi), 7);
	   w.setElementAt(new Point3D( 0+xi, 0+yi, 2+zi), 8);
   ...
   float eyeAndScreen(Dimension dim,int a)
   ...
   {  initPersp();
      ...
      d = 5000;
      //System.out.println(d * (xScrMin + xScrMax)/2+"    "+d * (yScrMin + yScrMax)/2);
      imgCenter = new Point2D(27, -26);
      for (int i=1; i<n; i++)
   ...
   void Setvp(Canvas cv, float dTheta, float dPhi, float fRho){
	   theta = dTheta;
	   phi = dPhi;
	   rho = fRho;
   }
   ...
   void printVP()
   {
	   System.out.println("theta="+theta+",phi="+phi+"rho="+rho);
   }
